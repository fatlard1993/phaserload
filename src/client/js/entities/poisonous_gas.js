...
phaserload.entities.poisonous_gas = function(x, y){
	Phaser.Sprite.call(this, phaserload.game, phaserload.toPx(x), phaserload.toPx(y), 'map', 'poisonous_gas_fill1');
...
phaserload.entities.poisonous_gas.create = function(x, y, spreadChance){
	...
	if(!poisonous_gas){
		...
		poisonous_gas.reset(phaserload.toPx(x), phaserload.toPx(y));
	...
	if(spreadChance !== undefined){
		poisonous_gas.spreadChance = spreadChance - util.randInt(5, 20);
...
phaserload.entities.poisonous_gas.spread = function(poisonous_gas){
	...
	if(pos.x - 1 > 0 && !surrounds.left && util.chance(spreadChance)){
		phaserload.setMapPos({ x: pos.x - 1, y: pos.y }, 'poisonous_gas', 'fill', null, spreadChance);
	}

	if(pos.x + 1 < phaserload.state.world.width && !surrounds.right && util.chance(spreadChance)){
		phaserload.setMapPos({ x: pos.x + 1, y: pos.y }, 'poisonous_gas', 'fill', null, spreadChance);
	}

	if(pos.y - 1 > 0 && !surrounds.top && util.chance(spreadChance)){
