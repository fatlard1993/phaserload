...
class SocketGame extends SocketRoom {
	constructor(options, server) {
		...
		this.options = Object.assign(
			{
				//todo make items and minerals configurable
	...
	addClient(client, name) {
		...
		const player = Object.assign(
			...
			phaserload.modes[this.options.mode].player || {},
			this.state.players[client.name],
		);

		if (player.credits instanceof Array) player.credits = jsUtil.randInt.apply(null, player.credits);
		if (!player.position) player.position = { x: jsUtil.randInt(1, this.state.world.width - 1), y: this.state.world.airGap };
	...
	broadcastState() {
		...
		playerNames.forEach(name => {
			if (!players[name]) return console.log('Cant find player:', players[name]);
	...
	playerMove({ name, x, y }) {
		...
		let orientation,
			lastOrientation = this.state.players[name].orientation;
		...
		setTimeout(() => {
			game.state.players[name].moving = false;
		}, this.state.players[name].moveTime);
