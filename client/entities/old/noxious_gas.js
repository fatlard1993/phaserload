...
phaserload.entities.noxious_gas.create = function (x, y, isNew, spawnChance, spreadChance) {
	if (isNew && spawnChance !== undefined && !jsUtil.chance(spawnChance)) return;
	...
	if (isNew) {
		noxious_gas.spreadChance = spreadChance !== undefined ? spreadChance : spawnChance !== undefined ? spawnChance - jsUtil.randInt(5, 17) : 100;
...
phaserload.entities.noxious_gas.spread = function (noxious_gas) {
	...
	if (gridPos.y - 1 > 0 && (surrounds.top === undefined || { purple_monster: 1, red_monster: 1 }[surrounds.top])) {
		if (jsUtil.chance(30)) phaserload.entities.noxious_gas.create(gridPos.x, gridPos.y - 1, 1, noxious_gas.spreadChance);
	}
