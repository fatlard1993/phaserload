...
export default class ConsoleEntity extends Phaser.GameObjects.Image {
	constructor() {
	...
	draw_small() {
		...
		dom.createElem('div', { textContent: `Health: ${jsUtil.toFixed(phaserload.player.health.available, 1, true)}%`, appendTo: this.elem });
		dom.createElem('div', { textContent: `Fuel: ${jsUtil.toFixed(phaserload.player.fuel.available, 1, true)}%`, appendTo: this.elem });
		dom.createElem('div', { textContent: `Cargo: ${jsUtil.toFixed(phaserload.player.cargoBay.available, 1, true)}%`, appendTo: this.elem });
		dom.createElem('div', { textContent: `$: ${jsUtil.toFixed(phaserload.player.credits, 2, true)}`, appendTo: this.elem });
			...
			appendChildren: ['Items', 'Cargo Bay', 'Drill Config', 'Settings', 'Help'].map((name, index) => {
				return dom.createElem('button', {
					...
					onPointerPress: evt => {
						...
						this[`draw_inventory_${jsUtil.toCamelCase(name.toLowerCase())}`]();
					},
				});
			}),
		});
	...
	draw_inventory_items() {
		...
		Object.keys(phaserload.player.inventory).forEach(name => {
			dom.createElem('div', { textContent: `${jsUtil.capitalize(name)}: ${phaserload.player.inventory[name]}`, appendTo: this.content });
		});
	}
	...
	draw_inventory_cargoBay() {
		...
		Object.keys(phaserload.player.cargoBay.material).forEach(name => {
			dom.createElem('div', {
				textContent: `${jsUtil.capitalize(name.split('_')[0])} ${jsUtil.capitalize(phaserload.options.mineralNames[name.split('_')[1]])}: ${phaserload.player.cargoBay.material[name]}`,
	...
	draw_inventory_drillConfig() {
		...
		Object.keys(phaserload.player.configuration).forEach(name => {
			dom.createElem('label', {
				textContent: `${jsUtil.capitalize(name.split('_')[0])}`,
				appendTo: this.content,
				appendChildren: phaserload.player.configuration[name].split(':~:').map(data => {
					return dom.createElem('div', { textContent: data });
				}),
			});
		});
	}
			...
			appendChildren: ['Price List', 'Fuel', 'Drill Parts', 'Items'].map((name, index) => {
				return dom.createElem('button', {
					...
					onPointerPress: evt => {
						...
						this[`draw_spaceco_${jsUtil.toCamelCase(name.toLowerCase())}`]();
					},
				});
			}),
		});
