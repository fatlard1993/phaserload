...
Game.states.end.prototype.create = function(){
	...
	text = Game.phaser.add.text(0, -100, 'Game over!', { font: '60px ' + Game.config.font, fill: Game.config.textColor });
	...
	text = Game.phaser.add.text(0, -100, (Game.loseReason === 'fuel' ? 'Ran out of fuel' : 'Killed by '+ Cjs.capitalize(Game.loseReason, 1, '_')) + ' At depth: '+ Game.loseDepth, { font: '40px ' + Game.config.font, fill: Game.config.textColor });
	text.updateTransform();
	text.x = Game.phaser.width / 2 - text.getBounds().width / 2;
	text.alpha = 0;
